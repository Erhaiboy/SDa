...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
                 cut_validation=False,imgids=None,test_imgids=None):
        roots = {}
        roots['train'] = {
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids,imgids=imgids,test_imgids=test_imgids)
    ...
    @classmethod
    def get_samples(cls, roots, ids_dataset=None,imgids=None,test_imgids=None):
        train_samples = []
        val_samples = []
        test_samples = []
            ...
            for index in range(len(ids)):
                ...
                if split == 'train':
                    if(imgids is None):
                        ...
                        # pass
                        if(str(img_id) in imgids):
                            train_samples.append(example)
                elif split == 'val':
                    val_samples.append(example)
                elif split == 'test':
                    if(test_imgids is None):
                        test_samples.append(example)
                    else:
                        if(str(img_id) in test_imgids):
                            test_samples.append(example)
