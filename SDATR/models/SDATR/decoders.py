...
from models.transformer_ensemble.decoderAttention import MultiHeadAttention,SelfMultiHeadAttention,ChannelAttention
from models.transformer_ensemble.utils import sinusoid_encoding_table, PositionWiseFeedForward
...
class DecoderLayer(Module):
    def __init__(self, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1, self_att_module=None,
        ...
        self.self_att = SelfMultiHeadAttention(d_model, d_k, d_v, h, dropout, can_be_stateful=True,
                                           attention_module=self_att_module,
                                           attention_module_kwargs=self_att_module_kwargs)
        self.enc_att = MultiHeadAttention(d_model, d_k, d_v, h, dropout, can_be_stateful=False,
                                          attention_module=enc_att_module,
                                          attention_module_kwargs=enc_att_module_kwargs)
        self.cha_att = ChannelAttention(d_model, d_k, d_v, h, dropout, can_be_stateful=True,
                                           attention_module=self_att_module,
                                           attention_module_kwargs=self_att_module_kwargs)
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        ...
        cha_att = self.cha_att(input)
        self_att = self.lnorm1(input + self.dropout1_1(self_att) + self.dropout1_2(cha_att))
