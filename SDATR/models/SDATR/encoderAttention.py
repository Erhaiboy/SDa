...
class RelativePosition(nn.Module):
    ...
    def forward(self, length_q, length_k):
        w,h=int(np.sqrt(length_q)),int(np.sqrt(length_q))
        range_vec_q = torch.arange(length_q)
        range_vec_k = torch.arange(length_k)
        dis_x=torch.abs(range_vec_q[None,:]%w-range_vec_k[:,None]%h)
        dis_y=torch.abs(range_vec_q[None,:]//w-range_vec_k[:,None]//h)

        ChebyshevDistance_mat=torch.max(dis_x,dis_y)
        ManhattanDistance_mat=dis_x+dis_y
        
        distance_mat_clipped = torch.clamp(ChebyshevDistance_mat, 0, self.max_relative_position)
...
class ChannelAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16):
        ...
        self.conv1 = nn.Conv2d(in_channels,in_channels//reduction,stride=1,kernel_size=1,padding=0)
        self.conv2 = nn.Conv2d(in_channels//reduction,out_channels,stride=1,kernel_size=1,padding=0)
...
class ScaledChannelAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, comment=None):
        ...
        self.channelblock=ChannelAttentionBlock(d_model,d_model)
        self.d_model = d_model
        self.d_k = d_k
        self.d_v = d_v
        self.h = h
